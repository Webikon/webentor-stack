...
| `starter-2.1.2` | starter + theme 2.1.1 → 2.1.2 | WP 7.0 runtime alignment, `package.json` only: rewrites the six declared `@wordpress/*` ranges to the versions WP 7.0 bundles, declares the five externalized imports that were never declared (`compose`/`data`/`element`/`hooks`/`html-entities`), drops the unused `@wordpress/dependency-extraction-webpack-plugin`, and syncs the changelogs (2.1.2). No `webentor-core` constraint change — `0.15.4` is a transparent in-range patch. Run `pnpm install` afterwards. |
| `starter-2.1.3` | starter + theme 2.1.2 → 2.1.3 | Project metadata moves to `.webikon/project.json` (schema v2); `.webentor/project.json` is retired. **Changelog-only — it declares no rules:** creating a file, deleting another and resolving `theme_path` are outside what ast-grep can do, so the migration syncs the changelogs (2.1.3) and documents the manual step. Migrate by running `init` from `webentor-setup` ≥ 1.2.0, which writes the new file, deletes the old one and resolves `theme_path` against the themes declaring `webentor-core`. No dependency ranges moved. |
